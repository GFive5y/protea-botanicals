...
import { RoleContext } from "../App"; // ← NEW: for auth-aware header button

export default function Landing() {
  const navigate = useNavigate();
  const ctx = useContext(RoleContext); // ← NEW
  const role = ctx?.role || "customer"; // ← NEW: safe fallback
    ...
    >
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .signin-btn:hover { background: rgba(255,255,255,0.2) !important; }
      `}</style>

      <div className="grain" />

      {/* ── NEW: Slim fixed header with auth-aware Sign In button ── */}
      <header
        style={{
          position: "fixed",
          top: 0,
      ...
      >
        ...
        <button
          ...
          onClick={() =>
            navigate(role && role !== "customer" ? "/loyalty" : "/account")
          }
          style={{
        ...
        >
          {role && role !== "customer" ? "My Account" : "Sign In"}
        </button>
      </header>
      ...
      >
        ...
        <div style={{ marginBottom: "32px", textAlign: "center" }}>
          <svg width="80" height="80" viewBox="0 0 80 80" className="hero-leaf">
            <defs>
        ...
        >
          ...
          technology, producing the cleanest, most consistent cannabis products
          available in South Africa. Every product is lab-certified, QR-verified
        ...
        >
          {cards.map((card) => (
            ...
            >
              <div
                style={{ fontSize: "32px", marginBottom: "16px", opacity: 0.6 }}
              >
                {card.icon}
              </div>
              <div
