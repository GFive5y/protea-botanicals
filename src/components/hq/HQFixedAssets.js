...
export default function HQFixedAssets() {
  ...
  if (loading) return (
    <div style={S.center}>
      <div style={S.spinner} />
      <span style={{ fontSize: 14, color: T.ink500 }}>Loading asset register{"\u2026"}</span>
    </div>
  );
    ...
    <div style={S.root}>
      ...
      <div style={S.bsNote}>
        ...
        {anyBehind && (() => {
          ...
          return <div style={{ fontSize: 12, color: '#b45309', marginTop: 10, padding: "8px 12px", background: T.warningLight, borderRadius: T.radius.sm, border: `1px solid ${T.warningBd}` }}>{"\u26A0"} <strong>{maxBehind} month{maxBehind !== 1 ? "s" : ""} of depreciation unposted.</strong> Click Run Depreciation and select each month to catch up. NBV is overstated until posted.</div>;
        })()}
      </div>
        ...
        <div style={S.overlay}>
          <div style={S.modal}>
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {runError && <div style={{ background: T.dangerLight, color: T.danger, padding: '10px 14px', borderRadius: T.radius.md, fontSize: 13, marginBottom: 12 }}>{"\u26A0\uFE0F"} {runError}</div>}
              {runResult && <div style={{ background: T.successLight, color: T.success, padding: '12px 16px', borderRadius: T.radius.md, fontSize: 13, marginBottom: 12 }}>{"\u2713"} Done {"\u2014"} {runResult.posted} entries posted ({fmt(runResult.total)} total){runResult.skipped > 0 && ` \u00b7 ${runResult.skipped} skipped (already posted or fully depreciated)`}</div>}
...
const S = {
  ...
  alertBehind: { background: T.warningLight, border: `1px solid ${T.warningBd}`, borderRadius: T.radius.md, padding: '12px 16px', marginBottom: 16, display: 'flex', flexDirection: 'column', gap: 4, fontSize: 13, color: T.warning },
  statsRow: { display: 'flex', gap: 12, marginBottom: 24 },
  statCard: { flex: 1, background: T.surface, border: `1px solid ${TK.ink200}`, borderRadius: T.radius.lg, padding: '14px 18px', boxShadow: T.shadow.sm },
  ...
  sectionCount: { fontSize: 12, fontWeight: 400, color: TK.ink500, background: TK.ink100, padding: '2px 10px', borderRadius: T.radius.full },
  thead: { display: 'flex', gap: 8, padding: '9px 16px', background: TK.ink100, borderRadius: `${T.radius.md} ${T.radius.md} 0 0`, border: `1px solid ${TK.ink200}`, borderBottom: 'none' },
  th: { fontSize: 11, fontWeight: 700, color: T.ink400, letterSpacing: '0.07em' },
  assetCard: { background: T.surface, border: `1px solid ${TK.ink200}`, borderTop: 'none', overflow: 'hidden', transition: 'border-color 0.15s, box-shadow 0.15s' },
  ...
  catBadge: { fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: T.radius.full, whiteSpace: 'nowrap', flexShrink: 0 },
  badge: { fontSize: 11, fontWeight: 700, letterSpacing: '0.05em', padding: '2px 8px', borderRadius: T.radius.full, whiteSpace: 'nowrap' },
  depBar: { width: '100%', height: 8, background: TK.ink200, borderRadius: T.radius.sm, position: 'relative', overflow: 'hidden' },
  depBarFill: { height: '100%', background: '#b91c1c', borderRadius: T.radius.sm, transition: 'width 0.3s' },
  depBarLabel: { position: 'absolute', right: 0, top: -1, fontSize: 9, color: TK.ink500, fontWeight: 600 },
  expandBox: { padding: '14px 16px 16px', background: T.bg, borderTop: `1px solid ${TK.ink100}` },
  expandGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 },
  expandItem: { display: 'flex', flexDirection: 'column', gap: 4 },
  expandLabel: { fontSize: 11, fontWeight: 700, color: T.ink400, letterSpacing: '0.07em' },
  expandVal: { fontSize: 13, fontWeight: 600, color: TK.ink700 },
  histRow: { display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderBottom: `1px solid ${TK.ink100}`, background: T.surface },
  emptyHistory: { textAlign: 'center', padding: '32px 20px', background: T.surface, border: `1px solid ${TK.ink200}`, borderRadius: `0 0 ${T.radius.md} ${T.radius.md}` },
  bsNote: { background: T.infoLight, border: `1px solid ${T.infoBd}`, borderRadius: T.radius.lg, padding: '18px 20px', marginBottom: 16 },
  ...
  modal: { background: T.surface, borderRadius: T.radius.lg, width: '100%', maxWidth: 680, maxHeight: '92vh', overflow: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' },
  ...
  fieldLbl: { fontSize: 11, fontWeight: 700, color: T.ink400, letterSpacing: '0.07em' },
  select: { padding: '8px 12px', borderRadius: T.radius.md, border: `1px solid ${TK.ink300}`, fontSize: 13, color: TK.ink900, background: T.surface, minWidth: 120 },
  previewBox: { background: TK.ink100, borderRadius: T.radius.md, padding: '14px 16px', marginBottom: 20 },
