...
const ACCENT = "#4338ca";
const ACCENT_LIT = "#EEF2FF";

// WP-UNIFY: local T palette removed — now imported from src/styles/tokens.js
...
export default function HQEmailLogs() {
  ...
  const sBtn = (active) => ({
    ...
    background: active ? ACCENT_LIT : "#fff",
    color: active ? ACCENT : T.ink500,
  });
              ...
              {filtered.map((l) => {
                ...
                return (
                  <React.Fragment key={l.id}>
                    ...
                    {isOpen && (
                      <tr>
                        <td colSpan={7} style={{ padding: "12px 18px", background: T.bg, borderBottom: `1px solid ${T.border}` }}>
                          <div style={{ fontSize: 11, color: T.ink700, lineHeight: 1.7 }}>
                            ...
                            <pre style={{ margin: "4px 0 0", padding: 10, background: "#fff", border: `1px solid ${T.border}`, borderRadius: 4, fontSize: 10, fontFamily: "DM Mono, monospace", overflow: "auto", maxHeight: 200 }}>
