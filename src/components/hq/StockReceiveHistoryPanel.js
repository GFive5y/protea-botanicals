...
function printReceipt(receipt, lines) {
  ...
  const html = `
    <!DOCTYPE html><html><head><title>${receipt.rcv_reference || "RCV"}</title>
    <style>
    ...
    <div class="header">
      <div>
        <div class="ref">${receipt.rcv_reference || "RECEIPT"}</div>
    ...
    <div class="footer">Generated by NuAi · ${new Date().toLocaleDateString("en-ZA")} · ${receipt.rcv_reference || ""}</div>
    </body></html>
  `;
...
export default function StockReceiveHistoryPanel({ tenantId, onReviewPrices }) {
  ...
  const loadLines = async (receiptId) => {
    ...
    try {
      ...
      if (e) throw e;
      setLineCache((prev) => ({ ...prev, [receiptId]: data || [] }));
    } catch (err) {
  ...
  const filtered = receipts.filter((r) => {
    ...
    return (
      (r.rcv_reference || "").toLowerCase().includes(q) ||
  ...
  const totalValue = receipts.reduce(
    (s, r) => s + parseFloat(r.total_value || 0),
    0,
  );
                ...
                return (
                  <React.Fragment key={r.id}>
                    ...
                    >
                      ...
                      >
                        {r.rcv_reference || r.id?.slice(0, 8)}
                      </td>
