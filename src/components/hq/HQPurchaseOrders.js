// HQPurchaseOrders.js v1.0 — WP-B: Purchase Order Flow (Import-Aware)
...
// ─── FX Rate Hook (mirrors HQSuppliers.js pattern) ───────────────────────────
function useFxRate() {
...
export default function HQPurchaseOrders() {
  ...
  const handleCreate = async () => {
    ...
    const itemInserts = lineItems.map((item) => ({
      ...
      line_total: parseFloat((item.unit_price_usd * item.qty).toFixed(4)),
  ...
  const handleAdvance = async (po, newStatus) => {
    ...
    if (newStatus === "ordered") {
      // Set expected arrival based on shipping mode
  ...
  const handleReceive = async (po) => {
    ...
    // Fetch line items to update inventory
    const { data: items } = await supabase
      .from("purchase_order_items")
      .select("*, supplier_products(name, sku, category)")
      .eq("po_id", po.id);
      ...
      for (const item of items) {
        ...
        if (existing) {
          await supabase
            ...
            .update({
              quantity_on_hand:
                (existing.quantity_on_hand || 0) + (item.quantity_ordered || 0),
            })
            .eq("id", existing.id);
        }
        // Note: if no inventory item exists, stock update is skipped (avoids schema conflicts).
        // Items can be mapped manually via HQ → Supply Chain if needed.
      }
    }
    ...
    <div style={{ fontFamily: "Jost, sans-serif", color: "#333" }}>
      ...
      {overdueCount > 0 && (
        ...
        >
          ...
          — expected arrival date has passed. Filter by status to review.
        </div>
      )}
              ...
              {step === 1 && (
                <div>
                    ...
                    >
                      ...
                      >
                        <div>
                          ...
                          >
                            {s.country} · {s.currency} · {s.contact_email || ""}
                          </div>
                        </div>
                        {selSupplier?.id === s.id && (
                  ...
                  {suppliers.length === 0 && (
                    ...
                    >
                      No suppliers found. Add suppliers via HQ → Supply Chain.
                    </div>
                  )}
                </div>
              )}
