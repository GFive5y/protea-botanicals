...
// v1.2: Added 7th sub-tab "📅 Campaigns" — create/edit/delete double_points_campaigns
//       Admin can set name, start_date, end_date, multiplier, is_active toggle
// v1.1: 6 sub-tabs: Earning Rules | Tiers | Economics | Referrals | QR Security | Channel Simulator
...
// ─── SUB-TAB 1–6 (unchanged from v1.1) ───────────────────────────────────────
// Keeping all existing tab components identical to v1.1
...
// ─── SUB-TAB 7: CAMPAIGNS (v1.2 NEW) ─────────────────────────────────────────
function TabCampaigns({ showToast }) {
  const [campaigns, setCampaigns] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [editId, setEditId] = useState(null); // null = new, uuid = edit
    ...
    <div>
      ...
      >
        ...
        <InfoBox colour={C.gold} bgColour={C.goldPale}>
          ...
          <em>on top of</em> their tier multiplier. A Gold tier customer (1.5×)
          during a 2× campaign earns <strong>1.5 × 2 = 3×</strong> base points.
  ...
  return (
    <div style={{ fontFamily: FONT_BODY, background: C.bg, minHeight: "100%" }}>
        ...
        >
          <div>
            ...
            >
              ...
              CheckoutPage, Loyalty, COGS and Pricing.
            </p>
          </div>
          {activeTab < 6 && (
            <button
          ...
          {SUB_TABS.map((tab, i) => (
            <button
              ...
              style={{
                ...
                borderBottom:
                  activeTab === i
                    ? `3px solid ${i === 6 ? C.gold : C.green}`
                    : "3px solid transparent",
                fontFamily: FONT_BODY,
                fontSize: 13,
                fontWeight: activeTab === i ? 600 : 400,
                color:
                  activeTab === i ? (i === 6 ? C.gold : C.green) : C.textMid,
                cursor: "pointer",
                whiteSpace: "nowrap",
      ...
      <div style={{ padding: "24px 28px", maxWidth: 900 }}>
        ...
        {activeTab === 6 && <TabCampaigns showToast={showToast} />}
      </div>

      {/* ── STICKY SAVE FOOTER (only for config tabs 0-5) ── */}
      {isDirty && activeTab < 6 && (
        <div
          style={{
            position: "sticky",
